...
let totalCount = 0;
let totalPrice = 0;
//토탈 상품 갯수, 토탈 금액 리프레시 이게 뭘까요? 슬롬꺼 따라치는중인데 잘 모르겠음.
const orderInfoRefresh = () => {
    totalCount = 0;
    totalPrice = 0;
    orderMenu.forEach((e) => {
        totalCount += e.count; // totalCount = totalCount + e.count
        totalPrice += e.price * e.count;
    });
    document.getElementById('totalQuantitySpan').innerText = totalCount;
    document.getElementById('totalPriceSpan').innerText = `${totalPrice.toLocalString()}원`;
}
...
const viewSelectMenu = (menu) => {
    ...
    <div class="middle">
        ...
        <div class="wrap__quantity">
            ...
            <span class="item_quantity" id="order_${menu.id}>${menu.count}">1</span>
            <button type="button" class="minus" onclick="minus(${menu.id})">-</button>
        </div>
    </div>
    <div class="bottom">
        <span>가격</span>
        <span id="order_price_${menu.id}">${menu.price.toLocalString()}원</span>>
    </div>
</div>`;
...
const selectMenu = (id) => {
    ...
    if (orderMenuItem == null) {
        ...
        document.getElementById(`order_price_${id}`).innerText = `${(orderMenuItem.price * orderMenuItem.count).toLocalString()}원`;
    }
    orderInfoRefresh();
}